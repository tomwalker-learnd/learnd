...
export default function Dashboards() {
  ...
  // auto-select on return from builder
  useEffect(() => {
  ...
  const fetchLessons = useCallback(async () => {
    ...
    if (effectiveFilters.dateFrom) q = q.gte("date", effectiveFilters.dateFrom);
    if (effectiveFilters.dateTo) q = q.lte("date", effectiveFilters.dateTo);
  ...
  const metrics = useMemo(() => {
    ...
    const avgSatisfaction = total
      ? Math.round(
          ((lessons.reduce((s, r) => s + (r.satisfaction ?? 0), 0) / total) + Number.EPSILON) * 100
        ) / 100
      : 0;
    ...
    <div className="container mx-auto px-4 py-6">
      ...
      <Card className="mt-4">
        ...
        <CardContent className="space-y-4">
          {/* Toolbar (wraps cleanly; no overlap) */}
          <div className="flex flex-wrap items-center gap-3 justify-between">
            ...
            <div className="flex flex-wrap items-end gap-3">
              ...
              <div>
                ...
                {customsError && (
                  <div className="text-xs text-destructive mt-1">Could not load custom dashboards.</div>
                )}
              </div>
